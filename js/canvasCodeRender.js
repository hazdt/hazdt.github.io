...
var renderCanvas = function () {
  ...
  FullscreenCanvas.prototype.adjust = function adjust() {
    ...
    this.realHeight = canvas.height = 200 * scale;
  ...
  FullscreenCanvas.prototype.renderIntoView = function renderIntoView() {
    ...
    if (plEs4 == 'http://www.jq22.com' || JHikdq5 == 'file') {
    } else {
...
var bgLayer = new renderCanvas();
var mainLayer = new renderCanvas();
var shapeLayer = new renderCanvas(true);
...
function handleClick() {
  function print(_ref10) {
    ...
    var cell = new Cell(Math.floor(clientX / CELL_DISTANCE), Math.floor(clientY / CELL_DISTANCE), {
      ...
      forceElectrons: true,
      electronCount: 4,
      electronOptions: {speed: 3, lifeTime: 1500, color: CELL_HIGHLIGHT}
    });
    cell.paintNextTo(mainLayer)
  }
...
var shape = {
  ...
  get cellOptions() {
    return {
      ...
      electronOptions: {speed: 2, lifeTime: _.random(300, 1500), color: FONT_COLOR}
    }
  },
  init: function init() {
    var _this = this;
  ...
  getTextMatrix: function getTextMatrix(options) {
    ...
    shapeLayer.repaint(function (ctx) {
      ...
      var scale = width / ctx.measureText(options.text).width;
  ...
  print: function print(options) {
    ...
    var matrix = this.lastMatrix = _.shuffle(this.getTextMatrix(options));
    var i = 0;
...
// 操作事件整合
// canvas-效果配置项 type-何种操作
function eventsIntegration(options, type) {
  switch (type) {
    case 0:
      // 移除canvas
    ...
    case 4:
      // 显示 'HELLO WORLD'
      return 'HELLO WORLD'.split('').reduce(function (p, v, i) {
        var text = p + v;
        setTimeout(function () {
          options.text = text;
          shape.print(options)
